...
$sql="update tblhumanresources set FirstName=:fname,LastName=:lname,Gender=:gender,Dob=:dob,Department=:department,Address=:address,City=:city,Country=:country,Phonenumber=:mobileno where id=:eid";
...
$query->execute();
$msg="Human Resources Manager record updated Successfully";
}
...
<html lang="en">
    <head>
        ...
        <!-- Title -->
        <title>Admin | Update Human Resources Manager</title>
...
.succWrap{
  ...
  <?php include('includes/header.php');?>
   ...
   <main class="mn-inner">
                <div class="row">
                    <div class="col s12">
                        <div class="page-title">Update human resources manager</div>
                    </div>
                        ...
                        <div class="card">
                            <div class="card-content">
                                <form id="example-form" method="post" name="updatemp">
                                    <div>
                                        <h3>Update Human Resources Manager Info</h3>
...
<?php 
$eid=intval($_GET['empid']);
$sql = "SELECT * from  tblhumanresources where id=:eid";
$query = $dbh -> prepare($sql);
$query -> bindParam(':eid',$eid, PDO::PARAM_STR);
$query->execute();
...
<label for="empcode">Human Resources Manager Code(Must be unique)</label>
<input  name="empcode" id="empcode" value="<?php echo htmlentities($result->HrId);?>" type="text" autocomplete="off" readonly required>
...
<?php $sql = "SELECT DepartmentName from tblhumanresources";
$query = $dbh -> prepare($sql);
$query->execute();
$results=$query->fetchAll(PDO::FETCH_OBJ);
$cnt=1;
