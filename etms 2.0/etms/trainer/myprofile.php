...
$sql="update tbltrainer set FirstName=:fname,LastName=:lname,Gender=:gender,Dob=:dob,Department=:department,Address=:address,City=:city,Country=:country,Phonenumber=:mobileno where TrId=:eid";
...
<html lang="en">
    <head>
        ...
        <!-- Title -->
        <title>Trainer | Update Trainer</title>
...
.succWrap{
  ...
  <?php include('includes/header.php');?>
   ...
   <main class="mn-inner">
                <div class="row">
                    <div class="col s12">
                        <div class="page-title">Update trainer</div>
                    </div>
...
<?php 
$eid=$_SESSION['emplogin'];
$sql = "SELECT * from  tbltrainer where TrId=:eid";
$query = $dbh -> prepare($sql);
$query -> bindParam(':eid',$eid, PDO::PARAM_STR);
$query->execute();
...
<label for="empcode">Trainer Code</label>
<input  name="empcode" id="empcode" value="<?php echo htmlentities($result->TrId);?>" type="text" autocomplete="off" readonly required>
...
<?php $sql = "SELECT DepartmentName from tbltrainer";
$query = $dbh -> prepare($sql);
$query->execute();
$results=$query->fetchAll(PDO::FETCH_OBJ);
$cnt=1;
