...
$sql="update tblcashmanager set FirstName=:fname,LastName=:lname,Gender=:gender,Dob=:dob,Department=:department,Address=:address,City=:city,Country=:country,Phonenumber=:mobileno where id=:eid";
...
<html lang="en">
    <head>
        ...
        <!-- Title -->
        <title>Admin | Update Cash Manager</title>
...
.succWrap{
  ...
  <?php include('includes/header.php');?>
   ...
   <main class="mn-inner">
                <div class="row">
                    <div class="col s12">
                        <div class="page-title">Update cash manager</div>
                    </div>
                        ...
                        <div class="card">
                            <div class="card-content">
                                <form id="example-form" method="post" name="updatemp">
                                    <div>
                                        <h3>Update Cash Manager Info</h3>
...
<?php 
$eid=intval($_GET['empid']);
$sql = "SELECT * from  tblcashmanager where id=:eid";
$query = $dbh -> prepare($sql);
$query -> bindParam(':eid',$eid, PDO::PARAM_STR);
$query->execute();
...
<label for="empcode">Cash Manager Code(Must be unique)</label>
<input  name="empcode" id="empcode" value="<?php echo htmlentities($result->CmId);?>" type="text" autocomplete="off" readonly required>
...
<?php $sql = "SELECT DepartmentName from tblcashmanager";
$query = $dbh -> prepare($sql);
$query->execute();
$results=$query->fetchAll(PDO::FETCH_OBJ);
$cnt=1;
